...
public class TransactionMapModel : ClassMap<TransactionModel>
{
    public TransactionMapModel()
    {
        ...
        Map(m => m.IdDirection).Index(4).Name(Res.Transaction.lDirection);
        Map(m => m.IdCcy).Index(5).Name(Res.Transaction.lCurrency);
        Map(m => m.DebtorIban).Index(6).Name(Res.Transaction.lDebtorIban);
        Map(m => m.BeneficiaryIban).Index(7).Name(Res.Transaction.lBeneficiaryIban);
        Map(m => m.Status).Index(8).Name(Res.Transaction.lStatus);
        Map(m => m.ExternalId).Index(9).Name(Res.Transaction.lExternalid);
    }
}
